(function ($) {
    "use strict";
      ...
      let loggedin_designation = ($("#loggedin_designation").val() || "").toLowerCase();
      ...
      function is_loggedin_waiter_designation() {
          return loggedin_designation === "waiter";
      }
      ...
      function enforce_waiter_lock_for_waiter_designation() {
          if(!is_loggedin_waiter_designation()){
              return;
          }
          let current_user_id = $("#user_id").val();
          let current_user_name = $("#user_name").val();
          $("#select_waiter").val(current_user_id).trigger("change");
          $("#select_waiter1").val(current_user_id).trigger("change");
          $("#select_waiter_name").val(current_user_name);
          $("#select_waiter, #select_waiter1").prop("disabled", true).addClass("self_order_skip");
          $(".no-need-for-waiter").hide();
      }

      enforce_waiter_lock_for_waiter_designation();
      ...
      function getDisplayTaxLabel(tax_name) {
          if (tax_name === "CGST") {
      ...
      function getDisplayPriceWithTaxInfo(base_price, tax_information) {
          let base_price_number = parseFloat(base_price);
          if (isNaN(base_price_number)) {
              base_price_number = 0;
          }

          let total_price = base_price_number;
          let tax_type = Number($("#tax_type").val());
          if (tax_type !== 1) {
              return total_price.toFixed(ir_precision);
          }

          let tax_info = [];
          if (Array.isArray(tax_information)) {
              tax_info = tax_information;
          } else if (typeof tax_information === "string" && tax_information !== "") {
              try {
                  tax_info = JSON.parse(tax_information);
              } catch (e) {
                  tax_info = [];
              }
          }
          ...
          if (tax_info.length > 0) {
              for (let i = 0; i < tax_info.length; i++) {
                  let this_tax = tax_info[i];
                  if (this_tax && checkTaxApply(this_tax.tax_field_name)) {
                      let tax_percentage = parseFloat(this_tax.tax_field_percentage);
                      if (!isNaN(tax_percentage)) {
                          total_price += (base_price_number * tax_percentage) / 100;
                      }
                  }
              }
          }
      ...
      function getDisplayPriceForPosItem(item_data) {
          let base_price = parseFloat(item_data.price);
          let selected_order_type_object = $(".main_top").find("button[data-selected=selected]");
          let order_type = selected_order_type_object.attr("data-id");
          if (order_type === "take_away_button") {
              base_price = parseFloat(item_data.price_take);
          } else if (order_type === "delivery_button") {
              base_price = parseFloat(item_data.price_delivery);
          }
          if (isNaN(base_price)) {
              base_price = 0;
          }
      ...
      function getEmailSeedStatus(){
          let email_send_auto_checker = Number($("#email_send_auto_checker").val());
          if ($("#check_send_mail").is(":checked")) {
              if(email_send_auto_checker==1){
                  $("#check_send_mail").prop('checked',true);
              }else{
                  $("#check_send_mail").prop('checked',false);
              }
               return 1;
          }else{
              if(email_send_auto_checker==1){
      ...
      function getWhatsappSeedStatus(){
          let whatsapp_send_auto_checker = Number($("#whatsapp_send_auto_checker").val());
          if ($("#check_send_whatsapp").is(":checked")) {
              if(whatsapp_send_auto_checker==1){
                  $("#check_send_whatsapp").prop('checked',true);
              }else{
                  $("#check_send_whatsapp").prop('checked',false);
              }
               return 1;
          }else{
              if(whatsapp_send_auto_checker==1){
      ...
      function checkEmailDisabled(customer_id){
          if(customer_id==1){
              $("#check_send_mail").prop('checked',false);
              $("#check_send_mail").prop('disabled',true);
          }else{
              let email_send_auto_checker = Number($("#email_send_auto_checker").val());
              if(email_send_auto_checker==1){
                    $("#check_send_mail").prop('checked',true);
                ...
                $("#check_send_mail").prop('disabled',false);
          }
      }
      function checkWhatsappDisabled(customer_id){
          if(customer_id==1){
              $("#check_send_whatsapp").prop('checked',false);
              $("#check_send_whatsapp").prop('disabled',true);
          }else{
              let whatsapp_send_auto_checker = Number($("#whatsapp_send_auto_checker").val()); 
              if(whatsapp_send_auto_checker==1){
                    $("#check_send_whatsapp").prop('checked',true);
                ...
                $("#check_send_whatsapp").prop('disabled',false);
          }
      }
  ...
  // purpose of this function is to delete data from sales table by using data from indexeddb
      ...
      function print_kot_popup_print(order_info,ignore_kot_update=0) {
          ...
          let invoice_print =``;
          console.log("[PRINT][KOT][POPUP] print_kot_popup_print called", {
              kot_print: kot_print,
              ignore_kot_update: ignore_kot_update,
              order_count: order_info ? order_info.length : 0
          });
          ...
          for (let key in order_info) {
              ...
              tmp_header_html+=`<h3 class="kot-print-title">`+txt_kot+`: `+order.kitchen_name+`</h3>`;
              tmp_header_html+=`
                                      `+inv_p_table+`
          ...
          if(need_print_popup!=1){     
            if(no_print!=1){
                ...
                if(!popup){
                    console.error("[PRINT][KOT][POPUP] window.open blocked", { need_print_popup: need_print_popup, no_print: no_print });
                    toastr['error']("KOT popup was blocked by browser. Allow popups for this site.", '');
                    return;
                }
                popup.document.write(invoice_print);
                popup.document.close();
                popup.focus();
                console.log("[PRINT][KOT][POPUP] popup opened and content written");
            }
      ...
      function push_online_for_kitchen(order_object,is_self_order,sale_no,is_print=0){
          if(is_loggedin_waiter_designation()){
              is_print = 0;
              console.log("[PRINT][KOT][PUSH_ONLINE] Skipped for waiter designation on save order");
          }
          ...
          $.ajax({
              ...
              success: function (data) {
                console.log("[PRINT][KOT][PUSH_ONLINE] add_kitchen_sale_by_ajax response", {
                    web_type: data.web_type,
                    print_type: data.print_type,
                    popup_count: (data.content_data_popup_print || []).length,
                    direct_count: (data.content_data_direct_print || []).length,
                    status: data.status,
      ...
      function push_online_update(orders){
          $.ajax({
              ...
              error:function(){
                  toastr['error']("Unable to place order online right now. Please retry.", '');
              }
          });
      }
      ...
      function print_kot_print(order_info,kot_print) {
          ...
          for(let k=0;k<total_kitchen_arr.length;k++){
              ...
              if(get_html){
                  ...
                  invoice_print+=`<h3 class="kot-print-title">`+txt_kot+`: `+this_kitchen_name+`</h3>`;
      ...
      function call_print_invoice(order_info,inv_qr_code_enable_status) {
        ...
        invoice_print+= `<!doctype html>
                ...
                <body>
                    <div id="wrapper">
                        <div id="receiptData">
                            <div id="receipt-data">
                                ...
                                <h3 class="invoice-print-title"> `+outlet_name+` </h3> 
                                `+text_no_str+`
                                    <p class="p_txt">
                    ...
                    <tr>
                        <td style="text-align:left;font-size:12px;"><b>` +order.sale_no+`</b></td>
                        <td style="text-align:right;font-size:12px;"><b>`+order_type+`</b></td>
                    </tr>   
                </table>
        ...
        if(Number(order.delivery_charge_actual_charge) && Number(order.order_type) !== 3){
            invoice_print+=`<tr>
 ...
 <hr style="border-bottom:1px solid black;margin: 0px;"> 
                <table class="table table-striped table-condensed">
                    <tbody>`;
        invoice_print+= ` <tr>
                                        <th  class="text_left" style="font-size:12px;"><b>`+inv_total+`</b>
                                        </th>
                                            <th class="text-right"><b style="font-size:12px;">
                                                `+getAmount(order.total_payable)+`</b>
                                            </th>
                                        </tr>`;
 ...
 invoice_print+= ` </table>
                ...
                <table class="table table-striped table-condensed">
                    <tbody>
                        <tr>`;
        invoice_print+= ` <tr>
                                        <th style="border-top: 2px solid;font-size:12px;"  class="text_left"><b>Total</b>
                                        </th>
                                            <th style="border-top: 2px solid;" class="text-right"><b style="font-size:12px;">
                                                `+getAmount(order.total_payable)+`</b>
                                            </th>
                                        </tr></tbody></table>`;
                      ...
                      if(data !== null) {
                          if(checkInternetConnection()){
                              ...
                              if (print_type_bill == "web_browser_popup") {
                                  ...
                                  console.log("[PRINT][BILL] direct_print requested", { sale_no: sale_no });
                                  $.ajax({
                                      ...
                                      success: function (data) {
                                          console.log("[PRINT][BILL] printSaleBillByAjax response", data);
                                          if (data.printer_server_url) {
                                              $.ajax({
                                                  ...
                                                  success: function (res) {
                                                      console.log("[PRINT][BILL] print_server success", res);
                                                  },
                                                  error: function (xhr, textStatus, errorThrown) {
                                                      console.error("[PRINT][BILL] print_server error", {
                                                          textStatus: textStatus,
                                                          errorThrown: errorThrown,
                                                          responseText: xhr && xhr.responseText ? xhr.responseText : ""
                                                      });
                                                      toastr['error']("Bill network print failed. Check printer server URL/network.", '');
                                                  },
                                              });
                                          } else {
                                              console.warn("[PRINT][BILL] Missing printer_server_url", data);
                                              toastr['error']("No network bill printer configured on this counter.", '');
                                          }
                                      },
                                      error: function (xhr, textStatus, errorThrown) {
                                          console.error("[PRINT][BILL] printSaleBillByAjax ajax error", {
                                              textStatus: textStatus,
                                              errorThrown: errorThrown,
                                              responseText: xhr && xhr.responseText ? xhr.responseText : ""
                                          });
                                          toastr['error']("Unable to fetch bill print payload.", '');
                                      },
                                  });
                              }
                          }else{
                              ...
                              if (is_invoice == 2) {
                                  ...
                                  let res = getSelectedOrderDetails(sale_no).then(function(data){
                                      ...
                                      if(response !== null) {
                                          ...
                                          checkEmailDisabled(response.customer_id);
                                          checkWhatsappDisabled(response.customer_id);
      ...
      $(document).on(
        ...
        function openProductEditModal(parent_id,item_name, id){
            ...
            for (let key1 in foundItems_variations) {
                ...
                let variation_display_price = getDisplayPriceWithTaxInfo(item_price, foundItems_variations[key1].tax_information);
    ...
    function setMergeTableFinal(objects,sale_no,table_text,merge_tables){
      ...
      $(document).on("click", ".place_order_operation", function (e) {
        let hidden_table_name = $("#hidden_table_name").val();
          ...
          if(is_login_checker==true){
              ...
              let waiter_name = '';
              if(is_loggedin_waiter_designation()){
                  waiter_id = $("#user_id").val();
                  waiter_name = $("#user_name").val();
                  $("#select_waiter").val(waiter_id).trigger("change");
                  $("#select_waiter1").val(waiter_id).trigger("change");
                  $("#select_waiter_name").val(waiter_name);
              }
              if(waiter_app_status!="Yes"){
      ...
      function is_mobile_money_payment_element($el) {
        let by_flag = ($el.attr("data-mobile-money") || "").toString().trim() === "1";
        if (by_flag) {
          return true;
        }
        let payment_method_name = ($el.attr("data-name") || $el.text() || "").toLowerCase().trim();
        return payment_method_name.indexOf("mobile") > -1 ||
          payment_method_name.indexOf("momo") > -1 ||
          (payment_method_name.indexOf("airtel") > -1 && payment_method_name.indexOf("money") > -1) ||
          (payment_method_name.indexOf("mtn") > -1 && payment_method_name.indexOf("money") > -1);
      }

      $(document).on("click", "#finalize_order_button", function (e, is_yopayments_paid) {
        let is_mobile_money_payment = is_mobile_money_payment_element($(".set_payment.active"));
        if (is_mobile_money_payment && !is_yopayments_paid) {
          let phone = $("#finalize_phone_number").val();
          let amount = $("#pay_amount_invoice_input").val();
          if (!phone) {
            toastr['error']("Please enter phone number", '');
            return;
          }
          initiate_yopayments(phone, amount);
          return;
        }
    ...
    //KOT
    function direct_print_server_request(printer_server_url, content_data, print_type, log_prefix) {
      let base = (printer_server_url || "").replace(/\/+$/, "");
      let endpoints = [
        base + "/printserver/irestora_printer_server.php",
        base + "/print_server/irestora_printer_server.php",
        base + "/irestora_printer_server.php"
      ];
      let idx = 0;

      function tryNext() {
        if (idx >= endpoints.length) {
          console.error(log_prefix + " direct print failed on all endpoints", { target_server_url: printer_server_url, endpoints: endpoints });
          toastr['error']("Direct printer request failed. Ensure printer service is reachable and allows CORS.", '');
          return;
        }
        ...
        $.ajax({
          ...
          success: function (res) {
            console.log(log_prefix + " direct print success", { endpoint: endpoint, responseText: res });
          },
          error: function (xhr, textStatus, errorThrown) {
            console.error(log_prefix + " direct print error", {
              endpoint: endpoint,
              textStatus: textStatus,
              errorThrown: errorThrown,
              responseText: xhr && xhr.responseText ? xhr.responseText : ""
    ...
    function relay_print_server_request(printer_server_url, content_data, print_type, log_prefix) {
      return $.ajax({
        url: base_url + "Authentication/relayPrintServer",
        method: "post",
        dataType: "json",
        data: {
          printer_server_url: printer_server_url,
          content_data: content_data,
          print_type: print_type
        },
        success: function (res) {
          console.log(log_prefix + " relay success", { target_server_url: printer_server_url, relay_response: res });
          if (res && res.status === "error") {
            let msg = (res.message || "").toLowerCase();
            if (msg.indexOf("could not connect") > -1 || msg.indexOf("failed to connect") > -1 || msg.indexOf("unable to connect") > -1) {
              console.warn(log_prefix + " relay unreachable, trying browser direct print fallback", res);
              direct_print_server_request(printer_server_url, content_data, print_type, log_prefix);
            }
          }
        },
        error: function (xhr, textStatus, errorThrown) {
          console.error(log_prefix + " relay error", {
            textStatus: textStatus,
            errorThrown: errorThrown,
            responseText: xhr && xhr.responseText ? xhr.responseText : ""
          });
          toastr['error']("Printer relay failed. Check server logs / printer server URL.", '');
        }
      });
    }
    ...
    function get_all_kitchen_ids_csv() {
      let kitchen_ids = [];
      $(".kitchen_ids").each(function () {
        kitchen_ids.push($(this).val());
      });
      return kitchen_ids.join(",");
    }

    function get_selected_kitchen_ids_csv() {
      let kitchen_ids = [];
      $(".kitchen_ids:checked").each(function () {
        kitchen_ids.push($(this).val());
      });
      return kitchen_ids.join(",");
    }
    ...
    function render_kot_item_selector(sale_no) {
      ...
      getSelectedOrderDetails(sale_no).then(function(order_info){
        let order_object = order_info;
        try {
          if (typeof order_object === "string") {
            order_object = JSON.parse(order_object);
          }
        } catch (e) {
          order_object = null;
        }
        if(!order_object || !order_object.items || !order_object.items.length){
          return;
        }
        let html_items = '<ul class="custom_ul">';
        for(let i = 0; i < order_object.items.length; i++){
          let item = order_object.items[i];
          if(!item || !item.food_menu_id){
            continue;
          }
          let menu_name = item.menu_name ? item.menu_name : ("Item #" + item.food_menu_id);
          let qty = item.qty ? item.qty : 0;
          let checkbox_id = "kot_item_checkbox_" + item.food_menu_id + "_" + i;
          html_items += '<li class="custom_li_kitchen">';
          html_items += '<input type="checkbox" class="kot_item_checkbox class_check" id="' + checkbox_id + '" value="' + item.food_menu_id + '" />';
          html_items += '<label class="label_c_kitchen" for="' + checkbox_id + '">' + menu_name + ' (' + qty + ')</label>';
          html_items += '</li>';
        }
        html_items += '</ul>';
        $("#kot_item_list_holder").html(html_items);
      });
    }

    function handle_kot_print_request(selected_order_no, kitchen_id, kot_print, selected_food_menu_ids) {
      $.ajax({
        ...
        data: {
          ...
          kot_print: kot_print,
          selected_food_menu_ids: selected_food_menu_ids ? selected_food_menu_ids : ""
        },
        success: function (data) {
          console.log("[PRINT][KOT] selectedKitchenPrinting response", data);
          if (data && data.status === "error") {
            toastr['error']((data.message || "Unable to prepare KOT print data"), '');
            return;
          }
          ...
          if (!popup_only_mode && !printed_to_network) {
    ...
    $(document).on("click", "#print_kot_modal", function (e) {
        ...
        let selected_order_no = $(".holder .order_details .single_order[data-selected=selected]").find(".running_order_order_number").text();
        console.log("[PRINT][KOT] print_kot_modal clicked", { selected_order_no: selected_order_no });
        if(checkInternetConnection()){
            let kitchen_id = get_selected_kitchen_ids_csv();
  
            let kot_print = $("#kot_print").val();
            if(kitchen_id){
                console.log("[PRINT][KOT] Requesting full-kitchen print", {
                    selected_order_no: selected_order_no,
                    kitchen_id: kitchen_id,
                    kot_print: kot_print
                });
                handle_kot_print_request(selected_order_no, kitchen_id, kot_print, "");
    ...
    $(document).on("click", "#print_selected_kot_modal", function (e) {
        if (is_loggedin_waiter_designation()) {
            return block_waiter_restricted_action();
        }
        if(!checkInternetConnection()){
            toastr['error']("Selected-item KOT printing requires online mode.", '');
            return;
        }
        let selected_order_no = $(".holder .order_details .single_order[data-selected=selected]").find(".running_order_order_number").text();
        let selected_food_menu_ids = [];
        $(".kot_item_checkbox:checked").each(function () {
            selected_food_menu_ids.push($(this).val());
        });
        if(!selected_food_menu_ids.length){
        ...
        let kot_print = $("#kot_print").val();
        handle_kot_print_request(selected_order_no, kitchen_id, kot_print, selected_food_menu_ids.join(","));
        $(this)
          .parent()
          .parent()
          .parent()
    ...
    $(document).on("click", ".kot_content_column .single_modifier", function () {
      ...
      function print_kot_button(kot_print){
        if(checkInternetConnection()){
            let sale_no = $(".print_kot_button").attr("data-sale_no");
            render_kot_item_selector(sale_no);
        ...
        if (sale.sale_date > getCurrentDate()) {
            if(!update_sale_id) {
                let order_info = {
                    ...
                    is_send_email:getEmailSeedStatus(),
                    is_send_whatsapp:getWhatsappSeedStatus(),
                    outlet_id: outlet_id,
    ...
    function close_order(sale_no, payment_method_type, paid_amount, due_amount,sub_total_discount_finalize) {
        ...
        let res = getSelectedOrderDetails(sale_no).then(function(order_info){
            ...
            order_object.send_email_status = getEmailSeedStatus();
            order_object.send_whatsapp_status = getWhatsappSeedStatus();
            ...
            getSelectedOrderDetailsRecentSale(sale_no).then(function(order_info){
                if(checkInternetConnection()){
                    $.ajax({
                        ...
                        success: function (data) {
                            console.log("[PRINT][INVOICE] printSaleByAjax response", data);
                            if (data.printer_server_url) {
                                $.ajax({
                                    url:
                                    data.printer_server_url +
                                    ...
                                    success: function (res) {
                                        console.log("[PRINT][INVOICE] print_server success", res);
                                    },
                                    error: function (xhr, textStatus, errorThrown) {
                                        console.error("[PRINT][INVOICE] print_server error", {
                                            textStatus: textStatus,
                                            errorThrown: errorThrown,
                                            responseText: xhr && xhr.responseText ? xhr.responseText : ""
                                        });
                                        toastr['error']("Invoice network print failed. Check printer server URL/network.", '');
                                    },
                                });
                            } else {
                                console.warn("[PRINT][INVOICE] Missing printer_server_url", data);
                                toastr['error']("No network invoice printer configured on this counter.", '');
                            }
                        },
                        error: function (xhr, textStatus, errorThrown) {
                            console.error("[PRINT][INVOICE] printSaleByAjax ajax error", {
                                textStatus: textStatus,
                                errorThrown: errorThrown,
                                responseText: xhr && xhr.responseText ? xhr.responseText : ""
                            });
                            toastr['error']("Unable to build invoice print payload.", '');
                        },
                    });
                }else{
                    call_print_invoice(order_info,inv_qr_code_enable_status);
            ...
            getSelectedOrderDetailsRecentSale(sale_no).then(function(order_info){
                if(checkInternetConnection()){
                    $.ajax({
                        ...
                        success: function (data) {
                            console.log("[PRINT][INVOICE] printSaleByAjax (resent) response", data);
                            if (data.printer_server_url) {
                                $.ajax({
                                    url:
                                    data.printer_server_url +
                                    ...
                                    success: function (res) {
                                        console.log("[PRINT][INVOICE] print_server (resent) success", res);
                                    },
                                    error: function (xhr, textStatus, errorThrown) {
                                        console.error("[PRINT][INVOICE] print_server (resent) error", {
                                            textStatus: textStatus,
                                            errorThrown: errorThrown,
                                            responseText: xhr && xhr.responseText ? xhr.responseText : ""
                                        });
                                        toastr['error']("Invoice reprint failed at network printer server.", '');
                                    },
                                });
                            } else {
                                console.warn("[PRINT][INVOICE] Missing printer_server_url for reprint", data);
                                toastr['error']("No network invoice printer configured for reprint.", '');
                            }
                        },
                        error: function (xhr, textStatus, errorThrown) {
                            console.error("[PRINT][INVOICE] printSaleByAjax (resent) ajax error", {
                                textStatus: textStatus,
                                errorThrown: errorThrown,
                                responseText: xhr && xhr.responseText ? xhr.responseText : ""
                            });
                            toastr['error']("Unable to fetch invoice data for reprint.", '');
                        },
                    });
                }else{
                    call_print_invoice(order_info,inv_qr_code_enable_status);
  ...
  <div>
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response && response.status === "error") {
              toastr['error']((response.message || "This action is not allowed."), '');
              return;
            }
            if (response) {
    ...
    $(document).on("change", "#select_waiter1", function (e) {
      if (is_loggedin_waiter_designation()) {
        let current_user_id = $("#user_id").val();
        $("#select_waiter1").val(current_user_id).trigger("change.select2");
        $("#select_waiter").val(current_user_id).trigger("change.select2");
        return;
      }
      let this_value = $(this).val();
    ...
    win.on('click', function(e){
      ...
      $(document).on("click", ".div_rectangular", function (e) {
          ...
          if(!is_active_action.length){
              // Fallback for layouts where active class state does not persist.
              is_active_action = this_action;
              this_action.addClass("div_rectangular_active");
              console.warn("[TABLE][CLICK] No active table row detected after click, using clicked row fallback");
          }
      ...
      $(document).on("click", ".get_area_table", function (e) {
          ...
          let id = $(this).attr('data-id');
          let floorName = $(this).text().trim();
          let this_floor_design = $(this).parent().find(".set_design").html();
          $(".table_bg").html(this_floor_design); 
          let renderedTableCount = $(".table_bg").find(".get_table_details").length;
          console.log("[TABLE][FLOOR] clicked", {
              floor_id: id,
              floor_name: floorName,
              rendered_table_count: renderedTableCount
          });
          if(!renderedTableCount){
              console.warn("[TABLE][FLOOR] No tables rendered for selected floor", {
                  floor_id: id,
                  floor_name: floorName
              });
          }
          ...
          if(checkInternetConnection()){
            ...
            console.log("[TABLE][FLOOR] kitchen status check queued", {
                floor_id: id,
                floor_name: floorName,
                tables_to_check: $(".table_bg").find(".get_table_details").length
            });
           }else{
                    //append ordered tables
      ...
      function set_active_payment(){
          $(".set_payment").each(function (i, obj) {
              ...
              let this_is_mobile_money = is_mobile_money_payment_element($(this));
              let id = Number($(this).attr("data-id"));
              if($(this).hasClass('active')){
                  $("#payment_preview").html(this_txt);
                  if (this_is_mobile_money) {
                      $(".mobile_money_div").attr("style", "display: block !important");
                  } else {
                      $(".mobile_money_div").attr("style", "display: none !important");
                  }
                  if(this_txt=="Cash"){
                      $(".cash_div").show();
      ...
      $(document).on("click", ".set_payment", function (e) {
          ...
          let is_mobile_money_payment = is_mobile_money_payment_element($(this));
          if (is_mobile_money_payment) {
              $(".mobile_money_div").attr("style", "display: block !important");
          } else {
              $(".mobile_money_div").attr("style", "display: none !important");
          }
      ...
      $(document).on("click", "#reset_table", function (e) {
          let write_reason_reset = $("#write_your_reason_reset").val() || "Please write the reason for resetting table";
          let activeTable = $(".table_bg").find(".div_rectangular.div_rectangular_active .get_table_details").first();
          let reset_table_id = activeTable.attr("data-id");
          let reset_sale_no = activeTable.attr("data-order_number");
          if (!reset_table_id || !reset_sale_no) {
              toastr['error']((please_select_open_order), '');
              return;
          }
          swal({
              title: warning + "!",
              text: write_reason_reset,
              type: "input",
              html: true,
              showCancelButton: true,
              closeOnConfirm: false,
          ...
          function(inputValue){
              if (inputValue === false) return false;
              if (inputValue === "") {
                  swal.showInputError(write_reason_reset);
                  return false
              }else{
                  swal.close();
                  $.ajax({
                      ...
                      data: {
                          reason: inputValue,
                          table_id: reset_table_id,
                          sale_no: reset_sale_no,
                          csrf_irestoraplus: csrf_value_,
                      },
                      success: function (response) {
                         ...
                         if (typeof response === "string") {
                             try {
                                 reset_response = JSON.parse(response);
                             } catch (e) {
                                 reset_response = { status: "success" };
                             }
                         }
                         if (reset_response && reset_response.status === "success") {
                             let reset_sale_no_txt = reset_response.sale_no ? reset_response.sale_no : reset_sale_no;
                             toastr['success'](("Order " + reset_sale_no_txt + " reset successfully"), '');
                         } else {
                             let reset_error_msg = (reset_response && reset_response.message) ? reset_response.message : a_error;
                             toastr['error']((reset_error_msg), '');
                         }
                         $(".get_area_table").eq(0).click();
                      },
                      error: function () {
      ...
      $(document).on("click", ".goto_to_payment", function (e) {
        ...
        if($(this).parent().parent().find('.row_box_item').length) {
            ...
            if (status == true) {
                ...
                checkEmailDisabled(customer_id);
                checkWhatsappDisabled(customer_id);
    ...
    function initiate_yopayments(phone, amount) {
      console.log("[YoPayments] Initiating mobile money payment", {
        phone: phone,
        amount: amount,
        order_id: $("#last_future_sale_id").val() || Date.now()
      });
      $("#finalize_order_button").prop("disabled", true);
      $("#finalize_order_button").html('<i class="fas fa-spinner fa-spin"></i> Processing Payment...');

      $.ajax({
        url: base_url + "Payments/initiate_mobile_money",
        method: "POST",
        data: {
          phone_number: phone,
          amount: amount,
        ...
        error: function (xhr, textStatus, errorThrown) {
          console.error("[YoPayments] initiate_mobile_money ajax error", {
            textStatus: textStatus,
            errorThrown: errorThrown,
            responseText: xhr && xhr.responseText ? xhr.responseText : ""
          });
          toastr['error']("Unable to initiate mobile money payment", '');
          $("#finalize_order_button").prop("disabled", false);
          $("#finalize_order_button").html('<i class="fas fa-file-invoice"></i> Submit');
        }
      });
      ...
      let pollInterval = setInterval(function () {
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (paymentFinalized) {
              console.log("[YoPayments] Poll response ignored (already finalized)", { ref: ref, response: response });
              return;
            }
            if (response.status === "success") {
              paymentFinalized = true;
              clearInterval(pollInterval);
              toastr['success']("Payment successful! Generating invoice.", '');
              $("#finalize_order_button").prop("disabled", false);
              $("#finalize_order_button").html('<i class="fas fa-file-invoice"></i> Submit');
          ...
          error: function (xhr, textStatus, errorThrown) {
            ...
            if (pollErrorCount >= 5) {
              clearInterval(pollInterval);
              toastr['error']("Unable to verify payment status. Please check network/API response in console.", '');
              $("#finalize_order_button").prop("disabled", false);
              $("#finalize_order_button").html('<i class="fas fa-file-invoice"></i> Submit');
            }
          }
