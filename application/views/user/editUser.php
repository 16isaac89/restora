            ...
            <div class="table-box">
                ...
                <div class="box-body">
                    ...
                    <div class="row">
                        <div class="col-sm-12 mb-2 col-md-12">
                            <div class="form-group">
                                ...
                                if(isset($user_details->work_schedule) && $user_details->work_schedule){
                                    $existing_schedule = json_decode($user_details->work_schedule,true);
                                }
                                ?>
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <?php foreach ($week_days as $day_key=>$day_label): ?>
                                            <?php
                                            $off_val = isset($existing_schedule[$day_key]['off']) ? (int)$existing_schedule[$day_key]['off'] : 0;
                                            $start_val = isset($existing_schedule[$day_key]['start']) ? escape_output($existing_schedule[$day_key]['start']) : '';
                                            $end_val = isset($existing_schedule[$day_key]['end']) ? escape_output($existing_schedule[$day_key]['end']) : '';
                                            if(isset($_POST['schedule_off'])){
                                                $off_val = isset($_POST['schedule_off'][$day_key]) ? 1 : 0;
                                            }
                                            if(isset($_POST['schedule_start'][$day_key])){
                                                $start_val = escape_output($_POST['schedule_start'][$day_key]);
                                            }
                                            if(isset($_POST['schedule_end'][$day_key])){
                                                $end_val = escape_output($_POST['schedule_end'][$day_key]);
                                            }
                                            ?>
                                            <tr>
                                                <td><?php echo escape_output($day_label); ?></td>
                                                <td>
                                                    <input type="checkbox" value="1" name="schedule_off[<?php echo escape_output($day_key); ?>]" <?php echo ($off_val==1)?'checked':''; ?>>
                                                </td>
                                                <td>
                                                    <input type="time" class="form-control" name="schedule_start[<?php echo escape_output($day_key); ?>]" value="<?php echo $start_val; ?>">
                                                </td>
                                                <td>
                                                    <input type="time" class="form-control" name="schedule_end[<?php echo escape_output($day_key); ?>]" value="<?php echo $end_val; ?>">
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                        </tbody>
                    ...
                    <div id="will_login_section"
                        ...
                        <div class="row">
                            <input type="hidden" name="order_receiving_id" value="">
