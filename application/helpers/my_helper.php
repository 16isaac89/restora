    ...
    if (!function_exists('sendTwilioWhatsAppMessage')) {
        /**
         * Send WhatsApp message using Twilio
         * @param string $phone - Recipient phone number (with country code, e.g., +1234567890)
         * @param string $body - Message body/text
         * @param string $company_id - Optional company ID, if not provided will use session
         * @return array - Response array with status and message
         */
        function sendTwilioWhatsAppMessage($phone, $body, $company_id = '') {
            try {
                $CI = &get_instance();
                if (!$company_id) {
                ...
                if (!$company_id) {
                    return array('status' => 'error', 'message' => 'Company ID not found');
                }
                
                $company = getCompanyInfo($company_id);
                if (!$company) {
                    return array('status' => 'error', 'message' => 'Company not found');
                }
                
                // Check if WhatsApp service provider is disabled
                $whatsapp_service_provider = isset($company->whatsapp_service_provider) ? $company->whatsapp_service_provider : '0';
                if ($whatsapp_service_provider == '0' || $whatsapp_service_provider == 0 || empty($whatsapp_service_provider)) {
                    return array('status' => 'error', 'message' => 'WhatsApp service is disabled. Please enable it in settings.', 'error_code' => 'SERVICE_DISABLED');
                }
                
                // Check if provider is set to Twilio (1)
                if ($whatsapp_service_provider != '1' && $whatsapp_service_provider != 1) {
                    return array('status' => 'error', 'message' => 'WhatsApp service provider is not set to Twilio. Current provider: ' . $whatsapp_service_provider, 'error_code' => 'WRONG_PROVIDER');
                }
                
                // Check if whatsapp_settings exists
                if (!isset($company->whatsapp_settings) || empty($company->whatsapp_settings)) {
                    return array('status' => 'error', 'message' => 'WhatsApp settings not found. Please configure WhatsApp settings first.');
                }
                ...
                if (json_last_error() !== JSON_ERROR_NONE) {
                    return array('status' => 'error', 'message' => 'Invalid WhatsApp settings format: ' . json_last_error_msg());
                }
                
                // Check enable status (handle both string and integer)
                $enableStatus = isset($whatsappSettings->enable_status) ? $whatsappSettings->enable_status : '0';
                if ($enableStatus != "1" && $enableStatus != 1) {
                    return array('status' => 'error', 'message' => 'WhatsApp messaging is disabled. Please enable it in settings.');
                }
                
                // Get and trim credentials
                $accountSid = isset($whatsappSettings->account_sid) ? trim($whatsappSettings->account_sid) : '';
                $authToken = isset($whatsappSettings->auth_token) ? trim($whatsappSettings->auth_token) : '';
                $whatsappNumber = isset($whatsappSettings->whatsapp_number) ? trim($whatsappSettings->whatsapp_number) : '';
                
                // Validate credentials
                if (empty($accountSid)) {
                    return array('status' => 'error', 'message' => 'Account SID is missing. Please configure it in WhatsApp settings.');
                }
                if (empty($authToken)) {
                    return array('status' => 'error', 'message' => 'Auth Token is missing. Please configure it in WhatsApp settings.');
                }
                if (empty($whatsappNumber)) {
                    return array('status' => 'error', 'message' => 'WhatsApp Number is missing. Please configure it in WhatsApp settings.');
                }
                
                // Validate Account SID format (should start with AC)
                if (substr($accountSid, 0, 2) !== 'AC' && strlen($accountSid) < 32) {
                    return array('status' => 'error', 'message' => 'Invalid Account SID format. Please check your Twilio Account SID.');
                }
                
                // Load Twilio library
                $twilioPath = FCPATH . 'Twilio/autoload.php';
                if (file_exists($twilioPath)) {
                    require_once $twilioPath;
                } else {
                    // Try alternative path
                    $twilioPath = './Twilio/autoload.php';
                    if (file_exists($twilioPath)) {
                        require_once $twilioPath;
                    } else {
                        return array('status' => 'error', 'message' => 'Twilio library not found at ' . FCPATH . 'Twilio/autoload.php');
                    }
                }
                
                // Initialize Twilio client with trimmed credentials - wrapped in try-catch
                try {
                    $client = new Twilio\Rest\Client($accountSid, $authToken);
                } catch (\Exception $initException) {
                    $initErrorMsg = $initException->getMessage();
                    if (strpos($initErrorMsg, 'Authenticate') !== false || strpos($initErrorMsg, '401') !== false || strpos($initErrorMsg, 'Unable to create record') !== false) {
                        return array(
                            'status' => 'error',
                            'message' => 'Authentication failed. Please verify your Account SID and Auth Token in WhatsApp settings.',
                            'error_code' => 'AUTH_ERROR'
                        );
                    }
                    return array(
                        'status' => 'error',
                        'message' => 'Failed to initialize Twilio client: ' . $initErrorMsg,
                ...
                $toNumber = preg_replace('/[\s\-]/', '', $toNumber);
                ...
                // Format WhatsApp number (ensure it has whatsapp: prefix)
                $fromNumber = $whatsappNumber;
                if (strpos($fromNumber, 'whatsapp:') === false) {
                ...
                try {
                    ...
                    if (strpos($errorMsg, 'Authenticate') !== false || strpos($errorMsg, '401') !== false || $errorCode == 401) {
                        return array(
                            'status' => 'error',
                            'message' => 'Authentication failed. Please verify your Account SID and Auth Token in WhatsApp settings.',
                            'error_code' => 'AUTH_ERROR',
                    ...
                    } elseif (strpos($errorMsg, 'not a valid WhatsApp number') !== false) {
                        return array(
                            'status' => 'error',
                            'message' => 'Invalid WhatsApp number format. Please check your WhatsApp number in settings.',
                ...
                if (strpos($errorMsg, 'Authenticate') !== false || strpos($errorMsg, '401') !== false || strpos($errorMsg, 'Unable to create record') !== false || $errorCode == 401) {
                    return array(
                        'status' => 'error',
                        'message' => 'Authentication failed. Please verify your Account SID and Auth Token in WhatsApp settings.',
                        'error_code' => 'AUTH_ERROR',
                ...
                return array(
                    'status' => 'error',
                    'message' => 'Twilio API error: ' . $errorMsg,
                ...
                // Check if it's a Twilio-related error
                if (strpos($errorClass, 'Twilio') !== false || strpos($errorMsg, 'Twilio') !== false) {
                    if (strpos($errorMsg, 'Authenticate') !== false || strpos($errorMsg, '401') !== false || strpos($errorMsg, 'Unable to create record') !== false) {
                        return array(
                            'status' => 'error',
                            'message' => 'Authentication failed. Please verify your Account SID and Auth Token in WhatsApp settings.',
                            'error_code' => 'AUTH_ERROR'
    ...
    if (!function_exists('sendWhatsAppMessageWithAttachment')) {
        /**
         * Send WhatsApp message with attachment - Router function that routes to appropriate provider
         * @param string $phone - Recipient phone number (with country code, e.g., +1234567890)
         * @param string $body - Message body/text
         * @param string $filePath - Path to the file to attach (can be local file path or URL)
         * @param string $company_id - Optional company ID, if not provided will use session
         * @return array - Response array with status and message
         */
        function sendWhatsAppMessageWithAttachment($phone, $body, $filePath = '', $company_id = '') {
            try {
                $CI = &get_instance();
                if (!$company_id) {
                    $company_id = $CI->session->userdata('company_id');
                ...
                // Check WhatsApp service provider and route accordingly
                $whatsapp_service_provider = isset($company->whatsapp_service_provider) ? $company->whatsapp_service_provider : '0';
                ...
                // Route to RCSoft WhatsApp API if provider is 2
                if ($whatsapp_service_provider == '2' || $whatsapp_service_provider == 2) {
                    return sendWhatsAppMessageWithAttachmentRCSoft($phone, $body, $filePath, $company_id);
                }
                
                // Otherwise, use Twilio (default behavior for backward compatibility)
                // Check if whatsapp_settings exists
                if (!isset($company->whatsapp_settings) || empty($company->whatsapp_settings)) {
                    return array('status' => 'error', 'message' => 'WhatsApp settings not found. Please configure WhatsApp settings first.');
                }
                ...
                // Handle attachment if file path is provided
                if (!empty($filePath)) {
                    // Check if it's a URL or local file
                    if (filter_var($filePath, FILTER_VALIDATE_URL)) {
                        // It's a URL
                        $messageParams['mediaUrl'] = array($filePath);
                    } else {
                        // It's a local file - need to upload to a publicly accessible URL first
                        // For local files, you might need to upload to a server or use Twilio's media storage
                        // For now, we'll assume it's a full URL or you need to handle file upload separately
                        if (file_exists($filePath)) {
                            // If file exists locally, you may need to make it accessible via URL
                            // For production, upload to a CDN or public storage first
                            $baseUrl = base_url();
                            // Try to convert local path to URL
                            $relativePath = str_replace(FCPATH, '', $filePath);
                            $fileUrl = $baseUrl . $relativePath;
                            $messageParams['mediaUrl'] = array($fileUrl);
                        } else {
                            return array('status' => 'error', 'message' => 'File not found: ' . $filePath);
                        }
                    }
                }
    ...
    if (!function_exists('sendWhatsAppMessageWithAttachmentRCSoft')) {
        /**
         * Send WhatsApp message with attachment using RCSoft WhatsApp API
         * @param string $phone - Recipient phone number (with country code, e.g., +1234567890)
         * @param string $body - Message body/text
         * @param string $filePath - Path to the file to attach (can be local file path or URL)
         * @param string $company_id - Optional company ID, if not provided will use session
         * @return array - Response array with status and message
         */
        function sendWhatsAppMessageWithAttachmentRCSoft($phone, $body, $filePath = '', $company_id = '') {
            try {
                $CI = &get_instance();
                if (!$company_id) {
                    $company_id = $CI->session->userdata('company_id');
                ...
                // Check if provider is set to RCSoft (2)
                if ($whatsapp_service_provider != '2' && $whatsapp_service_provider != 2) {
                    return array('status' => 'error', 'message' => 'WhatsApp service provider is not set to RCSoft. Current provider: ' . $whatsapp_service_provider, 'error_code' => 'WRONG_PROVIDER');
                }
                
                // Get credentials from company table
                $appKey = isset($company->whatsapp_app_key) ? trim($company->whatsapp_app_key) : '';
                $authKey = isset($company->whatsapp_authkey) ? trim($company->whatsapp_authkey) : '';
                
                // Validate credentials
                if (empty($appKey)) {
                    return array('status' => 'error', 'message' => 'WhatsApp App Key is missing. Please configure it in settings.');
                }
                if (empty($authKey)) {
                    return array('status' => 'error', 'message' => 'WhatsApp Auth Key is missing. Please configure it in settings.');
                }
                ...
                if (!empty($filePath)) {
                    ...
                    if (filter_var($filePath, FILTER_VALIDATE_URL)) {
                        ...
                        // It's a local file
                        if (file_exists($filePath)) {
                            // For local files, we need to use CURLFile for proper file upload
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    if (isset($responseData['status']) && $responseData['status'] == 'success') {
                        ...
                        return array(
                            'status' => 'error',
                            'message' => isset($responseData['message']) ? $responseData['message'] : 'Failed to send message',
                    ...
                    if ($httpCode >= 200 && $httpCode < 300) {
                        ...
                        return array(
                            'status' => 'error',
                            'message' => 'Failed to send message. Response: ' . $response,
    ...
    function getIPv4WithFormat($ipv_address){
        $ipv_address = trim((string)$ipv_address);
        if($ipv_address === ''){
            return '';
        }
        if(!preg_match('#^https?://#i', $ipv_address)){
            $ipv_address = 'http://'.$ipv_address;
        }
        return rtrim($ipv_address, '/').'/';
    }
    /**
     * send_email
     * @access public
    ...
    function generateCartItemsFromDB($items)
{
    ...
    function getHoldDetails($row_id) {
        $CI = & get_instance();
        $data = $CI->db->query("SELECT * FROM tbl_holds WHERE id='$row_id' AND del_status='Live'")->row();
        if($data){
            return $data;
        }else{
            return false;
    ...
    */
    ...
    function getAreaNameByTableNo($order_table_no,$outlet_id){
        if($order_table_no==''){
            return "";
        }
        $CI = & get_instance();
        $CI->db->select("a.area_name");
        $CI->db->from("tbl_tables as t");
        $CI->db->join('tbl_areas as a', 'a.id = t.area', 'left');
        $CI->db->where("t.name", $order_table_no);
        $CI->db->where("t.outlet_id", $outlet_id);
        $CI->db->where("t.del_status", "Live");
        $CI->db->where("a.del_status", "Live");
        $data = $CI->db->get()->row();
        return isset($data->area_name) && $data->area_name?$data->area_name:'';
    }
    function smsSendOnly($msg,$to){
        try {
                ...
                if($company->sms_service_provider==1){
                    try {
                        ...
                        if(empty($sid) || empty($token)){
                            error_log('SMS Error: Twilio Account SID or Auth Token is missing');
                            return false;
                        }
                        ...
                        if(empty($twilio_number)){
                            error_log('SMS Error: Twilio phone number is missing');
                            return false;
                        }
                        ...
                        error_log('SMS Twilio Error: ' . $errorMsg . ' (Code: ' . $errorCode . ')');
                        
                        // Check if it's an authentication error
                        if (strpos($errorMsg, 'Authenticate') !== false || strpos($errorMsg, '401') !== false || $errorCode == 401) {
                            error_log('SMS Error: Twilio authentication failed. Please verify your Account SID and Auth Token in SMS settings.');
                        } else {
                            error_log('SMS Error: Failed to send SMS via Twilio: ' . $errorMsg);
                        }
                        
                        // Return false to indicate failure without breaking the system
                        return false;
                    } catch (\Exception $e) {
                        $errorMsg = $e->getMessage();
                        error_log('SMS Error: Exception occurred while sending SMS via Twilio: ' . $errorMsg);
                        return false;
                    }
                    ...
                    try {
                        ...
                        if ($curlError) {
                            error_log('SMS Mobishastra cURL Error: ' . $curlError);
                            return false;
                        }
                        
                        if ($httpCode >= 400) {
                            error_log('SMS Mobishastra HTTP Error: ' . $httpCode . ' - Response: ' . $response);
                            return false;
                        }
                    } catch (\Exception $e) {
                        $errorMsg = $e->getMessage();
                        error_log('SMS Error: Exception occurred while sending SMS via Mobishastra: ' . $errorMsg);
                        return false;
                    }
                }
            }
            ...
            error_log('SMS Error: General exception in smsSendOnly: ' . $e->getMessage());
            return false;
        }
    }
