...
class Frontend extends Cl_Controller {
    ...
    public function reservations(){
        ...
        if ($this->input->post('submit')) {
            ...
            $this->form_validation->set_rules('special_request', lang('special_request'), 'max_length[250]');
            if ($this->form_validation->run() == TRUE) {
                ...
                $this->session->set_flashdata('exception', lang('reseration_confirm_message'));
            }
    ...
    public function singleItemOrder(){
        ...
        if ($food_menu_details && !empty($food_menu_details->parent_id) && $food_menu_details->parent_id > 0) {
            $parent = getFoodMenuDetails($food_menu_details->parent_id);
            $parent_name = ($parent && isset($parent->name)) ? $parent->name : '';
            $food_menu_details->name = $parent_name . ' ' . $food_menu_details->name;
        }
    ...
    /**
     * Get variation products by parent food menu id (for frontend menu item details when is_variation = 1).
     * Returns JSON with parent_name and list of variations (id, name, sale_price, tax_information, etc.).
     */
    public function getVariationsByParentId(){
        $parent_id = $this->input->post($this->security->xss_clean('parent_id'));
        if (empty($parent_id)) {
            $this->output->set_content_type('application/json')->set_output(json_encode(['status' => 'error', 'variations' => [], 'parent_name' => '']));
            return;
        }
        $parent = getFoodMenuDetails($parent_id);
        $parent_name = ($parent && isset($parent->name)) ? $parent->name : '';
        $variations = $this->Common_model->getAllByCustomId($parent_id, "parent_id", "tbl_food_menus", 'ASC');
        $list = [];
        if ($variations) {
            foreach ($variations as $v) {
                $list[] = [
                    'id' => $v->id,
                    'name' => $v->name,
                    'alternative_name' => isset($v->alternative_name) ? $v->alternative_name : '',
                    'sale_price' => $v->sale_price,
                    'tax_information' => isset($v->tax_information) ? $v->tax_information : '',
                    'parent_id' => $v->parent_id,
                    'product_type' => isset($v->product_type) ? $v->product_type : '1',
                    'display_name' => $parent_name . ' ' . $v->name,
                    'calories' => isset($v->calories) ? $v->calories : '',
                ];
            }
        }
        $response = ['status' => 'success', 'parent_name' => $parent_name, 'variations' => $list];
        $this->output->set_content_type('application/json')->set_output(json_encode($response));
    }
    ...
    public function paymentSetting($id = '') {
        if (htmlspecialcharscustom($this->input->post('submit'))) {
            ...
            $field_yopayments  = htmlspecialcharscustom($this->input->post('field_yopayments'));
            $field_yopayments_v  = htmlspecialcharscustom($this->input->post('field_yopayments_v'));
            $field_yopayments_key_1  = htmlspecialcharscustom($this->input->post('field_yopayments_key_1'));
            $field_yopayments_key_2  = htmlspecialcharscustom($this->input->post('field_yopayments_key_2'));
            if($field_yopayments==1){
                $this->form_validation->set_rules('field_yopayments_key_1', "API Username", "required");
                $this->form_validation->set_rules('field_yopayments_key_2', "API Password", "required");
            }
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['field_yopayments'] = $field_yopayments;

                $data['field_2_v'] = $field_2_v;
                $data['field_3_v'] = $field_3_v;
                $data['field_4_v'] = $field_4_v;
                $data['field_yopayments_v'] = $field_yopayments_v;
                ...
                $data['field_yopayments_key_1']  = $field_yopayments_key_1;
                $data['field_yopayments_key_2']  = $field_yopayments_key_2;
                //pre($data);
    ...
    public function whatsappSetting($id = '') {
        if (htmlspecialcharscustom($this->input->post('submit'))) {
            $this->form_validation->set_rules('whatsapp_service_provider', "WhatsApp Service Provider", "required");
            
            if ($this->form_validation->run() == TRUE) {
                $whatsapp_service_provider = htmlspecialcharscustom($this->input->post($this->security->xss_clean('whatsapp_service_provider')));
                $data = array();
                $data['whatsapp_service_provider'] = $whatsapp_service_provider;
               
                // Handle provider-specific settings
                if ($whatsapp_service_provider == '1') {
                    // Twilio settings
                    $whatsapp_settings = array();
                    $whatsapp_settings['account_sid'] = htmlspecialcharscustom($this->input->post($this->security->xss_clean('account_sid')));
                    $whatsapp_settings['auth_token'] = htmlspecialcharscustom($this->input->post($this->security->xss_clean('auth_token')));
                    $whatsapp_settings['whatsapp_number'] = htmlspecialcharscustom($this->input->post($this->security->xss_clean('whatsapp_number')));
                    $whatsapp_settings['enable_status'] = htmlspecialcharscustom($this->input->post($this->security->xss_clean('enable_status')));
                    $data['whatsapp_settings'] = json_encode($whatsapp_settings);
                } else if ($whatsapp_service_provider == '2') {
                    // RCSoft settings
                    $data['whatsapp_app_key'] = htmlspecialcharscustom($this->input->post($this->security->xss_clean('whatsapp_app_key')));
                    $data['whatsapp_authkey'] = htmlspecialcharscustom($this->input->post($this->security->xss_clean('whatsapp_authkey')));
                }
                
                $this->Common_model->updateInformation($data, $id, "tbl_companies");
                $this->session->set_flashdata('exception', lang('update_success'));
                redirect('Frontend/whatsappSetting');
            } else {
                $data = array();
