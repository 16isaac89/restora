...
class Authentication extends Cl_Controller {
    ...
    public function __construct() {
        ...
        $this->User_model->ensureUserScheduleAndSessionSchema();
    ...
    private function aggregateKotItemsForOutput($items) {
        ...
        foreach ($items as $item) {
            ...
            $tmp_qty = isset($item->tmp_qty) ? (float)$item->tmp_qty : 0;
            $display_qty = $this->getKotItemDisplayQty($item);
            if($display_qty <= 0){
                continue;
            }
        ...
        foreach ($grouped as $group_key => $group_item) {
            if(isset($group_item->_has_pending_tmp_qty) && (int)$group_item->_has_pending_tmp_qty === 1){
                $grouped[$group_key]->qty = (float)$group_item->tmp_qty;
                $grouped[$group_key]->is_print = 1;
            }
  ...
  public function frontend() {
                    ...
                    if($company_info){
                        if($company_info->is_active==1){
                            ...
                            if($is_payment_clear=="Yes" && $is_block=="No"){
                                ...
                                $company_info_session['sms_send_auto'] = $company_info->sms_send_auto;
                                $company_info_session['email_send_auto'] = $company_info->email_send_auto;
                                $company_info_session['whatsapp_send_auto'] = $company_info->whatsapp_send_auto;
                                ...
                                if(str_rot13($company_info->language_manifesto)=="fgjgldkfg"){
                                    ...
                                    $this->trackUserSessionLogin($user_information->id,$company_info->id,$user_information->outlet_id);
    ...
    public function logOut() {
        ...
        $this->session->unset_userdata('user_session_track_id');
        ...
        $this->session->unset_userdata('sms_send_auto');
        $this->session->unset_userdata('email_send_auto');
        $this->session->unset_userdata('whatsapp_send_auto');
     ...
     * @param int
     */
    private function trackUserSessionLogin($user_id,$company_id,$outlet_id=0){
        $login_time = date("Y-m-d H:i:s");
        $this->db->query("UPDATE tbl_user_sessions SET logout_at='$login_time', status='Closed', duration_minutes=TIMESTAMPDIFF(MINUTE, login_at, '$login_time') WHERE user_id='$user_id' AND status='Open'");
        $data = array();
        $data['user_id'] = $user_id;
        $data['company_id'] = $company_id;
        $data['outlet_id'] = $outlet_id;
        $data['login_at'] = $login_time;
        $data['status'] = 'Open';
        $data['ip_address'] = $this->input->ip_address();
        $data['user_agent'] = substr((string)$this->input->user_agent(),0,250);
        $data['del_status'] = 'Live';
        $this->db->insert('tbl_user_sessions', $data);
        $this->session->set_userdata('user_session_track_id', $this->db->insert_id());
    }
    /**
     ...
     * @return void
     */
    private function trackUserSessionLogout(){
        $user_id = $this->session->userdata('user_id');
        if(!$user_id){
            return;
        }
        $logout_time = date("Y-m-d H:i:s");
        $session_track_id = $this->session->userdata('user_session_track_id');
        if($session_track_id){
            $this->db->query("UPDATE tbl_user_sessions SET logout_at='$logout_time', status='Closed', duration_minutes=TIMESTAMPDIFF(MINUTE, login_at, '$logout_time') WHERE id='$session_track_id' AND user_id='$user_id'");
        }else{
            $this->db->query("UPDATE tbl_user_sessions SET logout_at='$logout_time', status='Closed', duration_minutes=TIMESTAMPDIFF(MINUTE, login_at, '$logout_time') WHERE user_id='$user_id' AND status='Open'");
        }
    }
    ...
    public function SMSSetting($id='') {
        ...
        if (htmlspecialcharscustom($this->input->post('submit'))) {
            ...
            if($sms_service_provider==1){
                ...
                $this->form_validation->set_rules('field_3_0','App Key', "required|max_length[250]");
                $this->form_validation->set_rules('field_3_1','Auth Key', "required|max_length[250]");
            }
            if ($this->form_validation->run() == TRUE) {
                $sms_info_json = array();
                ...
                // For provider 3, also save to company table directly for backward compatibility
                if($sms_service_provider==3){
                    $company_data = array();
                    $company_data['whatsapp_app_key'] = $sms_info_json['field_3_0'];
                    $company_data['whatsapp_authkey'] = $sms_info_json['field_3_1'];
                    $this->Common_model->updateInformation($company_data, $id, "tbl_companies");
                }
  ...
  public function setOutlet($outlet_id,$company_id){
    ...
    public function selectedKitchenPrinting(){
        ...
        if($this->session->userdata('designation')=="Waiter"){
            echo json_encode(array(
                ...
                'print_type' => 'KOT',
                'content_data_popup_print' => array(),
                'content_data_direct_print' => array()
            ));
            return;
        }
        ...
        $selected_sales_detail_ids_raw = trim((string)$this->input->post('selected_sales_detail_ids'));
        $selected_sales_detail_ids = array();
        if($selected_sales_detail_ids_raw!==''){
            $selected_sales_detail_ids = array_map('intval', array_filter(array_map('trim', explode(',', $selected_sales_detail_ids_raw))));
        }
        $selected_sales_detail_ids = array_values(array_unique(array_filter($selected_sales_detail_ids)));

        if(!$selected_order_no || !$kitchen_id){
            echo json_encode(array(
                'status' => 'error',
                'message' => 'Missing order number or kitchen selection',
                'print_type' => 'KOT',
                'content_data_popup_print' => array(),
                'content_data_direct_print' => array()
            ));
        ...
        if(!is_array($printers_popup_print)){
            $printers_popup_print = array();
        }

        $printers_direct_print = $this->Common_model->getSelectedPrinters($selected_order_no,$kitchen_id,2);
        if(!is_array($printers_direct_print)){
            $printers_direct_print = array();
        }
        ...
        $is_printing_return = 1;
            ...
            if(!$sale_details || !isset($sale_details->id)){
                echo json_encode(array(
                    'status' => 'error',
                    'message' => 'Sale details not found for selected order',
                    'print_type' => 'KOT',
                    'content_data_popup_print' => array(),
                    'content_data_direct_print' => array()
                ));
            ...
            foreach ($printers_popup_print as $ky=>$value){
                if(isset($value->id) && $value->id){
                    ...
                    $sale_items = $this->aggregateKotItemsForOutput($sale_items);
                    if($sale_items){
            ...
            $printing_kot_mode = ($web_type==1 || $web_type==="web_browser_popup") ? "web_browser_popup" : "direct_print";
            if($printing_kot_mode==="web_browser_popup"){
                echo json_encode(array(
                    'status' => 'success',
                    'message' => 'KOT popup print mode active',
                    'printing_kot_mode' => $printing_kot_mode,
                    'print_type' => 'KOT',
                    'content_data_popup_print' => $printers_popup_print,
                    'content_data_direct_print' => array()
                ));
            }else{
                if($is_printing_return!=1){
                    ...
                    $return_data['message'] = '';
                    $return_data['printing_kot_mode'] = $printing_kot_mode;
                    ...
                    echo json_encode(array(
                        'status' => 'success',
                        'message' => 'No kitchen print items found',
                        'printing_kot_mode' => $printing_kot_mode,
                        'print_type' => 'KOT',
                        'content_data_popup_print' => $printers_popup_print,
                        'content_data_direct_print' => array()
    ...
    public function getKotTrackingItems(){
        $this->output->set_content_type('application/json');
        $selected_order_no = htmlspecialcharscustom($this->input->post($this->security->xss_clean('selected_order_no')));
    ...
    public function getKotTrackingSummary(){
        $this->output->set_content_type('application/json');
        $sale_nos_raw = trim((string)$this->input->post('sale_nos'));
        $sale_nos = array_filter(array_map('trim', explode(',', $sale_nos_raw)));
        $summary = array();
        foreach ($sale_nos as $sale_no){
            ...
            $printed_count = 0;
            foreach ($items as $item){
                $is_printed = isset($item->manual_print_confirmed) && (int)$item->manual_print_confirmed === 1;
                if($is_printed){
                    $printed_count++;
                }else{
                    $pending_count++;
    ...
    public function markKotPrinted(){
        $this->output->set_content_type('application/json');
        $sale_no = htmlspecialcharscustom($this->input->post($this->security->xss_clean('sale_no')));
        $sale_details = getKitchenSaleDetailsBySaleNo($sale_no);
        if(!$sale_details || !isset($sale_details->id)){
            echo json_encode(array('status' => 'error', 'message' => 'Sale details not found'));
            return;
        }
        $this->db->set('manual_print_confirmed', 1);
    ...
    public function markKotItemPrinted(){
        $this->output->set_content_type('application/json');
        $sales_details_id = (int)$this->input->post($this->security->xss_clean('sales_details_id'));
        if(!$sales_details_id){
            echo json_encode(array('status' => 'error', 'message' => 'Missing KOT item'));
            return;
        }
        $this->db->set('manual_print_confirmed', 1);
    ...
    public function relayPrintServer(){
        $this->output->set_content_type('application/json');

        $printer_server_url = trim((string)$this->input->post('printer_server_url'));
        $content_data = (string)$this->input->post('content_data');
        $print_type = trim((string)$this->input->post('print_type'));

        if(!$printer_server_url || !$content_data || !$print_type){
            echo json_encode(array(
                'status' => 'error',
                'message' => 'Missing printer_server_url, content_data or print_type'
            ));
            return;
        }

        $base_url = rtrim($printer_server_url, '/');
        $candidate_requests = array();
        $candidate_requests[] = array(
            'method' => 'POST',
            'url' => $base_url . '/printserver/irestora_printer_server.php',
        ...
        $candidate_requests[] = array(
            'method' => 'POST',
            'url' => $base_url . '/print_server/irestora_printer_server.php',
            'body' => http_build_query(array(
                'content_data' => $content_data,
                'print_type' => $print_type
            ))
        ...
        $candidate_requests[] = array(
            'method' => 'POST',
            'url' => $base_url . '/irestora_printer_server.php',
            'body' => http_build_query(array(
                'content_data' => $content_data,
                'print_type' => $print_type
            ))
        ...
        foreach($candidate_requests as $candidate){
            $relay_url = $candidate['url'];
            $relay_method = $candidate['method'];
            $post_fields = $candidate['body'];
            $response_body = '';
            $http_code = 0;
            $transport_error = '';
            ...
            if(function_exists('curl_init')){
                ...
                if($relay_method === 'POST'){
                    $context_opts['header'] = "Content-Type: application/x-www-form-urlencoded\r\n";
                    $context_opts['content'] = $post_fields;
                }
                $context = stream_context_create(array('http' => $context_opts));
                $response_body = @file_get_contents($relay_url, false, $context);
                if($response_body === false){
                    $transport_error = 'Unable to connect to printer server';
                    $response_body = '';
                }
            }
            ...
            if(!$transport_error && $http_code < 400){
                ...
                echo json_encode(array(
                    ...
                    'http_code' => $http_code,
                    'attempts' => $attempts,
                    'printer_response' => $decoded ? $decoded : $response_body
                ));
                return;
            }

            $last_error_message = $transport_error ? $transport_error : ('Printer server returned HTTP '.$http_code);
        ...
        echo json_encode(array(
            'status' => 'error',
            'message' => $last_error_message ? $last_error_message : 'Failed to reach printer server',
            'relay_url' => $last_url,
    ...
    public function changeReservation() {
        ...
        $return_data['msg'] = lang('reservation_status_change');
    ...
    public function getReservations(){
        ...
        $html_content = '<table id="datatable111111" class="table_register_details top_margin_15"> <thead><tr>
                        <th class="ir_w_12">'.lang('name').'</th>
                        <th class="ir_w_9">'.lang('phone').'</th>
                        <th class="ir_w_11">'.lang('email').'</th>
                        <th class="ir_w_9">'.lang('number_of_guest').'</th>
                        <th class="ir_w_9">'.lang('reservation_date').'</th>
                        <th class="ir_w_1">'.lang('reservation_type').'</th>
                        <th>'.lang('special_request').'</th>
                        <th class="ir_w_1">'.lang('status').'</th>
                        <th class="ir_w_1">'.lang('actions').'</th>
                    </tr> </thead>
                    <tbody>';
    ...
    color: white;
    padding: 6px;
    border-radius: 11px; " class="form-control change_status_reservation">'.$options.'</select></td>
