...
class User extends Cl_Controller {
    ...
    public function __construct() {
        ...
        if($segment_2=="users"){
            ...
            $function = "delete";
        }elseif($segment_2=="sessionTracking"){
            $function = "view";
        }else{
    ...
    public function addEditUser($encrypted_id = "") {
        ...
        if (htmlspecialcharscustom($this->input->post('submit'))) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $user_info['order_receiving_id'] = '';
                $user_info['kitchens'] = $kitchens;
                $week_days = array('monday','tuesday','wednesday','thursday','friday','saturday','sunday');
                $schedule_starts = $this->input->post('schedule_start');
                $schedule_ends = $this->input->post('schedule_end');
                $schedule_off = $this->input->post('schedule_off');
                $work_schedule = array();
                foreach ($week_days as $day){
                    $is_off = (isset($schedule_off[$day]) && $schedule_off[$day]=='1') ? 1 : 0;
                    $start_time = isset($schedule_starts[$day]) ? htmlspecialcharscustom($schedule_starts[$day]) : '';
                    $end_time = isset($schedule_ends[$day]) ? htmlspecialcharscustom($schedule_ends[$day]) : '';
                    if($is_off){
                        $start_time = '';
                        $end_time = '';
                    }
                    $work_schedule[$day] = array(
                        'off' => $is_off,
                        'start' => $start_time,
                        'end' => $end_time
                    );
                }
                $user_info['work_schedule'] = json_encode($work_schedule);
     ...
     * @param string
     */
    public function sessionTracking($encrypted_id='') {
        $company_id = $this->session->userdata('company_id');
        $user_id = '';
        if($encrypted_id!=''){
            $user_id = $this->custom->encrypt_decrypt($encrypted_id, 'decrypt');
        }else{
        ...
        $data = array();
        $data['sessions'] = $this->User_model->getUserSessions($company_id,$user_id,$start_date,$end_date);
        $data['users'] = $this->Common_model->getAllByCompanyIdForDropdown($company_id, "tbl_users");
        $data['selected_user_id'] = $user_id;
        $data['start_date'] = $start_date;
        $data['end_date'] = $end_date;
        $data['main_content'] = $this->load->view('user/sessionTracking', $data, TRUE);
        $this->load->view('userHome', $data);
    }
     /**
