...
class Sale extends Cl_Controller {
    ...
    public function __construct() {
        ...
        $this->load->library('zatca_invoice');
        $this->load->model('Kitchen_model');
        $this->load->model('Waiter_model');
        $this->load->model('Master_model');
        $this->load->library('form_validation');
        $this->Common_model->ensureKitchenPrintTrackingSchema();
    ...
    public function refund($encrypted_id = "") {
        ...
        if (htmlspecialcharscustom($this->input->post('submit'))) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                // Submit Credit Note to ZATCA Phase-2 (if original invoice was submitted)
                $this->submitCreditNoteToZatca($id);
    ...
    public function POS($user_id='', $outlet_id='',$sale_id=""){
        ...
        $data['tables_for_pos'] = $this->Common_model->getAllByOutletId($outlet_id, 'tbl_tables');
    ...
    public function add_expense_by_ajax() {
        if(!checkAccess("142","add")){
            echo json_encode(array(
                'status' => false,
                'message' => lang('menu_not_permit_access')
            ));
            return;
        }
    ...
    public function add_kitchen_sale_by_ajax(){
        ...
        if($status_creating_invocie==false){
            ...
            if($this->session->userdata('designation')=="Waiter"){
                $data['waiter_id'] = $this->session->userdata('user_id');
            }
            $data['outlet_id'] = $this->session->userdata('outlet_id');
            $data['company_id'] = $this->session->userdata('company_id');
            ...
            if($sale_id>0 && count($order_details->items)>0){
                ...
                foreach($order_details->items as $key_counter=>$item){
                    ...
                    $holds_details_id = '';
                    if($sale_id){
                        $check_exist_item = null;
                        if(isset($item->item_previous_id) && $item->item_previous_id){
                            $this->db->select('*');
                            $this->db->from('tbl_kitchen_sales_details');
                            $this->db->where('sales_id', $sale_id);
                            $this->db->where('previous_id', $item->item_previous_id);
                            $this->db->where('del_status', 'Live');
                            $check_exist_item = $this->db->get()->row();
                        }
                        if(!$check_exist_item){
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $printed_sales_detail_ids = array();
                foreach ($printers_popup_print as $ky=>$value){
                    if(isset($value->id) && $value->id){
                        ...
                        foreach($sale_items as $single_item_by_sale_id){
                            $printed_sales_detail_ids[] = (int)$single_item_by_sale_id->sales_details_id;
                ...
                foreach ($printers_direct_print as $ky=>$value){
                    ...
                    $printed_sales_detail_ids = array_values(array_unique(array_filter($printed_sales_detail_ids)));
                    if(!empty($printed_sales_detail_ids)){
                        $this->db->set('is_print', 2);
                        $this->db->set('print_count', 'COALESCE(print_count,0)+1', false);
                        $this->db->where_in('id', $printed_sales_detail_ids);
                        $this->db->update('tbl_kitchen_sales_details');

                        $this->db->set('is_print', 2);
                        $this->db->where_in('sales_details_id', $printed_sales_detail_ids);
                        $this->db->update('tbl_kitchen_sales_details_modifiers');
                    }
                    ...
                    if($web_type=="web_browser_popup" || $web_type=="web_browser" || $web_type==1 || $web_type=="1"){
                        $return_data['printer_server_url'] = '';
                        $return_data['content_data_popup_print'] = $printers_popup_print;
                        $return_data['content_data_direct_print'] = array();
                        $return_data['print_type'] = "KOT";
                        $return_data['web_type'] = $web_type;
    ...
    public function resetTableData(){
        $outlet_id = $this->session->userdata('outlet_id');
        $reason = escape_output($this->input->post('reason'));
        $sale_no = escape_output($this->input->post('sale_no'));
        $table_id = escape_output($this->input->post('table_id'));

        if(!$sale_no || !$table_id){
            echo json_encode(array('status' => 'error', 'message' => 'Missing table or order selection'));
            return;
        }

        $select_kitchen_row = $this->db->select('id,sale_no')
        ...
        if(!$select_kitchen_row){
            echo json_encode(array('status' => 'error', 'message' => 'Selected order not found'));
            return;
        }
        ...
        $this->db->delete("tbl_kitchen_sales", array("id" => $select_kitchen_row->id, "outlet_id" => $outlet_id));
        $this->db->delete("tbl_running_order_tables", array("sale_no" => $sale_no, "table_id" => $table_id, "outlet_id" => $outlet_id));
        $this->db->delete("tbl_orders_table", array("sale_id" => $select_kitchen_row->id));

        $event_txt = "<b>Table Reset, Reason: " . $reason . "</b><br>";
        $event_txt .= "Affected Order: " . $sale_no . ", Table ID: " . $table_id;
        putAuditLog($this->session->userdata('user_id'), $event_txt, "Reset Table", date('Y-m-d H:i:s'));

        echo json_encode(array('status' => 'success', 'sale_no' => $sale_no, 'table_id' => $table_id));
    }
    ...
    public function push_online(){
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            // Generate invoice PDF if email or WhatsApp is enabled
            $invoice_pdf_path = '';
            if(($send_email_status == 1 || $send_whatsapp_status == 1) && $customer && $customer->id != 1){
                $invoice_pdf_path = $this->generateInvoicePDFFile($sales_id);
            }
            
            // Send SMS
            if($send_sms_status==1 && $customer && $customer->id != 1){
...
We hope to see you again!";
            ...
            if($send_email_status == 1 && $customer && $customer->id != 1 && $customer->email){
                $email_subject = "Invoice from ".$outlet_name." - Order #".$order_details->sale_no;
                $email_body = "Dear ".$customer->name.",

Thank you for visiting ".$outlet_name."!

Your order details:
- Order Number: ".$order_details->sale_no."
- Date & Time: ".$order_details->date_time."
- Total Amount: ".getAmtCustom($order_details->total_payable)."
- Paid Amount: ".getAmtCustom($order_details->paid_amount)."
- Due Amount: ".getAmtCustom($order_details->due_amount)."
...
".$outlet_name;
                ...
                // Check if PDF path is a file path (not URL)
                $is_file_path = $invoice_pdf_path && (strpos($invoice_pdf_path, 'http') !== 0) && file_exists($invoice_pdf_path);
                if($is_file_path){
                    sendEmailOnlyAZ($email_subject, $email_body, $customer->email, $invoice_pdf_path, basename($invoice_pdf_path), $company_id);
                } else if($invoice_pdf_path && strpos($invoice_pdf_path, 'http') === 0){
                    // If it's a URL, send email without attachment (or you can implement URL attachment)
                    sendEmailOnlyAZ($email_subject, $email_body."\n\nInvoice: ".$invoice_pdf_path, $customer->email, '', '', $company_id);
                } else {
                    sendEmailOnlyAZ($email_subject, $email_body, $customer->email, '', '', $company_id);
                }
            }
            
            // Send WhatsApp with PDF attachment
            if($send_whatsapp_status == 1 && $customer && $customer->id != 1 && $customer->phone){
                $whatsapp_body = "Hi ".$customer->name.", thank you for visiting ".$outlet_name.". 

Your order details:
Order #: ".$order_details->sale_no."
Date: ".$order_details->date_time."
Total: ".getAmtCustom($order_details->total_payable)."
Paid: ".getAmtCustom($order_details->paid_amount)."
Due: ".getAmtCustom($order_details->due_amount)."
...
We hope to see you again!";
                ...
                if($invoice_pdf_path){
                    ...
                    if(strpos($invoice_pdf_path, 'http') === 0){
                        // It's already a URL
                        sendWhatsAppMessageWithAttachment($customer->phone, $whatsapp_body, $invoice_pdf_path, $company_id);
                    } else if(file_exists($invoice_pdf_path)){
                        // Convert local path to URL for WhatsApp
                        $pdf_url = base_url().str_replace(FCPATH, '', $invoice_pdf_path);
                        sendWhatsAppMessageWithAttachment($customer->phone, $whatsapp_body, $pdf_url, $company_id);
                    } else {
                        // Send without attachment
                        sendTwilioWhatsAppMessage($customer->phone, $whatsapp_body, $company_id);
                    }
                } else {
                    sendTwilioWhatsAppMessage($customer->phone, $whatsapp_body, $company_id);
                }
            ...
            // Clean up PDF/HTML file after sending (optional - comment out if you want to keep files)
            if($invoice_pdf_path && strpos($invoice_pdf_path, 'http') !== 0 && file_exists($invoice_pdf_path)){
                // Optionally delete the file after sending, or keep it for records
                // Uncomment the line below if you want to delete files after sending
     ...
     * Loads data once and passes directly to library (no redundant DB queries)
     * 
     * @access private
     * @param int $sales_id Sale ID
     * @return void
     */
    private function submitToZatca($sales_id) {
        "use strict";
        try {
            $outlet_id = $this->session->userdata('outlet_id');
            
            // Load outlet ZATCA configuration (single query)
            $outlet_data = $this->db->select('is_zatca_enable, zatca_outlet, zatca_token')
                                     ->from('tbl_outlets')
                                     ->where('id', $outlet_id)
            ...
            // Check if ZATCA enabled
            if (!$outlet_data || $outlet_data->is_zatca_enable != 1) {
                return; // ZATCA not enabled, skip
            }
            
            // Load sale data with items (optimized single query)
            $sale_data = $this->loadSaleDataForZatca($sales_id);
            if (!$sale_data) {
                log_message('error', 'ZATCA: Sale data not found for ID: ' . $sales_id);
                return;
            }
            
            // Submit to ZATCA (pass data directly, no DB queries in library)
            $zatca_result = $this->zatca_invoice->submitInvoice($sales_id, $sale_data, $outlet_data);
            
            // Log result
            if ($zatca_result['success']) {
                log_message('info', 'ZATCA: Invoice submitted successfully for sale ID: ' . $sales_id);
            } else {
                log_message('error', 'ZATCA: Submission failed for sale ID: ' . $sales_id . ' - ' . $zatca_result['message']);
            }
            
        } catch (Exception $e) {
            log_message('error', 'ZATCA: Exception for sale ID: ' . $sales_id . ' - ' . $e->getMessage());
            
            // Save exception to database
            $this->saveZatcaException($sales_id, $e->getMessage(), $e->getTraceAsString());
        }
    }
    
    /**
     * Save ZATCA exception to database
     * 
     * @access private
     * @param int $sales_id Sale ID
     ...
     * @return void
     */
    private function saveZatcaException($sales_id, $exception_message, $trace) {
        "use strict";
        ...
        try {
            ...
            $zatca_data = array(
                ...
                'api_response' => null,
                'error_message' => 'Exception in submitToZatca: ' . $exception_message,
                'error_details' => $trace
            );
            ...
            $this->db->where('id', $sales_id)->update('tbl_sales', $update_data);
            
        } catch (Exception $e) {
            log_message('error', 'Failed to save ZATCA exception: ' . $e->getMessage());
        }
    }
    
    /**
     * Load sale data for ZATCA (Optimized - minimal queries)
     * 
     * @access private
     * @param int $sales_id Sale ID
     * @return object|null Sale data with items and customer
     */
    private function loadSaleDataForZatca($sales_id) {
        "use strict";
        // Load sale header
        $sale_data = $this->db->select('*')
                               ->from('tbl_sales')
                               ->where('id', $sales_id)
                               ->where('del_status', 'Live')
                               ->get()
        ...
        // Load items (single query)
        $sale_data->items = $this->db->select('*')
        ...
        // Load customer (single query, only if not walk-in)
        if ($sale_data->customer_id && $sale_data->customer_id != 1) {
            $sale_data->customer = $this->db->select('id, name, address')
                                             ->from('tbl_customers')
                                             ->where('id', $sale_data->customer_id)
                                             ->get()
                                             ->row();
        } else {
            $sale_data->customer = null;
        }
    ...
    /**
     * Submit Credit Note to ZATCA Phase-2 (for refunds)
     * Handles ZATCA credit note submission after refund processing
     * 
     * @access private
     * @param int $sales_id Sale ID
     * @return void
     */
    private function submitCreditNoteToZatca($sales_id) {
        "use strict";
        try {
            $outlet_id = $this->session->userdata('outlet_id');
            
            // Load outlet ZATCA configuration
            $outlet_data = $this->db->select('is_zatca_enable, zatca_outlet, zatca_token')
                                     ->from('tbl_outlets')
                                     ->where('id', $outlet_id)
                                     ->where('del_status', 'Live')
            ...
            // Load original sale data with ZATCA info
            $original_sale = $this->db->select('*')
                                      ->from('tbl_sales')
                                      ->where('id', $sales_id)
                                      ->where('del_status', 'Live')
                                      ->get()
            ...
            if (!$original_sale) {
                log_message('error', 'ZATCA Credit Note: Sale data not found for ID: ' . $sales_id);
                return;
            }
            
            // Check if original invoice was submitted to ZATCA
            if (!$original_sale->zatca_response || $original_sale->zatca_status != 'reported' && $original_sale->zatca_status != 'cleared') {
                log_message('info', 'ZATCA Credit Note: Original invoice not submitted to ZATCA, skipping credit note for sale ID: ' . $sales_id);
                return; // Original invoice not submitted, skip credit note
            }
            
            // Load refund data
            $refund_data = $this->loadRefundDataForZatca($sales_id);
            if (!$refund_data) {
                log_message('error', 'ZATCA Credit Note: Refund data not found for ID: ' . $sales_id);
                return;
            }
            
            // Submit Credit Note to ZATCA
            $zatca_result = $this->zatca_invoice->submitCreditNote(
                $sales_id, 
                $refund_data, 
                $original_sale, 
                $outlet_data
            ...
            // Log result
            if ($zatca_result['success']) {
                log_message('info', 'ZATCA Credit Note: Submitted successfully for sale ID: ' . $sales_id);
            } else {
                log_message('error', 'ZATCA Credit Note: Submission failed for sale ID: ' . $sales_id . ' - ' . $zatca_result['message']);
            }
            
        } catch (Exception $e) {
            log_message('error', 'ZATCA Credit Note: Exception for sale ID: ' . $sales_id . ' - ' . $e->getMessage());
        }
    }
    
    /**
     * Load refund data for ZATCA Credit Note
     * 
     * @access private
     * @param int $sales_id Sale ID
     * @return object|null Refund data with items
     */
    private function loadRefundDataForZatca($sales_id) {
        "use strict";
        // Load sale with refund info
        $sale_data = $this->db->select('*')
                               ->from('tbl_sales')
                               ->where('id', $sales_id)
                               ->where('del_status', 'Live')
        ...
        if (!$sale_data || !$sale_data->refund_content || !$sale_data->total_refund) {
            return null; // No refund data
        }
        
        // Parse refund items
        $refund_items = json_decode($sale_data->refund_content);
        if (!$refund_items || empty($refund_items)) {
            return null;
        }
        ...
        foreach ($refund_items as $item) {
            $item_total = ($item->price * $item->refund_qty) - ($item->discount ?? 0);
            $refund_subtotal += $item_total;
            $refund_tax += ($item->vat ?? 0) * $item->refund_qty;
            $refund_discount += ($item->discount ?? 0);
        }
        
        // Prepare refund data object
        $refund_data = new stdClass();
        $refund_data->sale_no = $sale_data->sale_no;
        $refund_data->outlet_id = $sale_data->outlet_id;
        $refund_data->refund_date_time = $sale_data->refund_date_time;
        $refund_data->total_refund = $sale_data->total_refund;
        $refund_data->refund_subtotal = $refund_subtotal;
        $refund_data->refund_tax = $refund_tax;
        $refund_data->refund_discount = $refund_discount;
        $refund_data->refund_items = $refund_items;
        $refund_data->customer_id = $sale_data->customer_id;
        
        // Load customer if exists
        if ($sale_data->customer_id && $sale_data->customer_id != 1) {
            $refund_data->customer = $this->db->select('id, name, address')
                                               ->from('tbl_customers')
                                               ->where('id', $sale_data->customer_id)
                                               ->get()
                                               ->row();
        } else {
            $refund_data->customer = null;
        }
     ...
     * @access private
     * @return string - Path to generated PDF file or URL
     * @param int
     */
    private function generateInvoicePDFFile($sale_id){
        try {
            ...
            $invoice_dir = FCPATH.'assets/invoices/';
            if(!is_dir($invoice_dir)){
                createDirectory('assets/invoices');
            }
            ...
            $pdf_filename = 'invoice_'.$sale_id.'_'.time().'.pdf';
            $pdf_path = $invoice_dir.$pdf_filename;
            
            // Try to use mPDF library
            try {
                // Check if mPDF is available via Composer (vendor/autoload.php)
                $vendor_autoload = FCPATH.'vendor/autoload.php';
                if(file_exists($vendor_autoload)){
                    require_once $vendor_autoload;
                }
                
                // Check if mPDF class exists (mPDF v8.x uses \Mpdf\Mpdf namespace)
                if(class_exists('\Mpdf\Mpdf')){
                    // Use mPDF v8.x (Composer version - compatible with PHP 8.0-8.3)
                    $mpdf = new \Mpdf\Mpdf([
                        'mode' => 'utf-8',
                        'format' => 'A4',
                        'orientation' => 'P',
                    ...
                    // Fallback: Use old mPDF class if available (legacy support)
                    $mpdf = new mPDF('utf-8', 'A4', 0, '', 10, 10, 10, 10);
                    $mpdf->WriteHTML($invoice_html);
                    $mpdf->Output($pdf_path, 'F');
            ...
            $html_filename = 'invoice_'.$sale_id.'_'.time().'.html';
            $html_path = $invoice_dir.$html_filename;
            file_put_contents($html_path, $invoice_html);
            
            $html_url = base_url().'assets/invoices/'.$html_filename;
            $command = "wkhtmltopdf --page-size A4 --encoding UTF-8 --quiet \"".$html_url."\" \"".$pdf_path."\" 2>&1";
            ...
            // Last resort: Return URL to HTML file (can be used as attachment URL)
            return base_url().'assets/invoices/'.$html_filename;
    ...
    public function getBillDetails(){
        ...
        if($this->session->userdata('designation')=="Waiter"){
            echo json_encode(array(
                'status' => 'error',
                'message' => 'This action is not allowed for waiter accounts'
            ));
            return;
        }
    ...
    public function closeRegister()
    {
        ...
        $register_row = $this->db->select('id')
            ->from('tbl_register')
            ->where('outlet_id', $outlet_id)
            ->where('counter_id', $counter_id)
            ->where('opening_balance_date_time', $opening_date_time)
        ...
        $register_id = (isset($register_row->id) && $register_row->id) ? $register_row->id : 0;
        $encrypted_register_id = $register_id ? $this->custom->encrypt_decrypt($register_id, 'encrypt') : '';
        $response = array(
            'status' => 'success',
            'register_id' => $register_id,
            'register_encrypted_id' => $encrypted_register_id,
            'user_id' => $user_id
        );
