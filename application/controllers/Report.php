...
class Report extends Cl_Controller {
    ...
    public function __construct() {
        ...
        if($segment_2!="todayReport"){
            if($segment_2=="registerReport"){
                ...
                $controller = "314";
                $function = "view";
            }elseif($segment_2=="cashierZReport"){
                $controller = "314";
                $function = "view";
    ...
    /**
     * Cashier Session Z Report
     * @access public
     * @return void
     * @param string
     */
    public function cashierZReport($encrypted_id = '')
    {
        $data = array();
        $outlet_id  = isset($_POST['outlet_id']) && $_POST['outlet_id']?$_POST['outlet_id']:'';
        if(!$outlet_id){
        ...
        if($encrypted_id){
            ...
            $data['register_info'] = $this->Report_model->getRegisterInformationById($id,$outlet_id);
            if(!$data['register_info']){
                $this->session->set_flashdata('exception_er', lang('menu_not_permit_access'));
                redirect('Report/registerReport');
            }
            $data['main_content'] = $this->load->view('report/cashierZReport', $data, TRUE);
            $this->load->view('userHome', $data);
            return;
        }
        ...
        $data['main_content'] = $this->load->view('report/cashierZReportSearch', $data, TRUE);
        $this->load->view('userHome', $data);
    }
        /**
     * register Report
    ...
    private function normalizeDateTimeInput($date_time = '', $end_of_minute = false) {
        if (!$date_time) {
            return '';
        }

        $normalized_date_time = str_replace('T', ' ', trim($date_time));
        $timestamp = strtotime($normalized_date_time);
