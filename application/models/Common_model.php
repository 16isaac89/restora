...
class Common_model extends CI_Model {
    ...
    /**
     * ensure kitchen print tracking schema
     * @access public
     * @return void
     */
    public function ensureKitchenPrintTrackingSchema() {
        $print_count_column = $this->db->query("SHOW COLUMNS FROM tbl_kitchen_sales_details LIKE 'print_count'")->row();
        if(!$print_count_column){
            $this->db->query("ALTER TABLE `tbl_kitchen_sales_details` ADD `print_count` INT(11) NOT NULL DEFAULT 0 AFTER `is_print`");
        }
        $manual_confirmed_column = $this->db->query("SHOW COLUMNS FROM tbl_kitchen_sales_details LIKE 'manual_print_confirmed'")->row();
        if(!$manual_confirmed_column){
            $this->db->query("ALTER TABLE `tbl_kitchen_sales_details` ADD `manual_print_confirmed` TINYINT(1) NOT NULL DEFAULT 0 AFTER `print_count`");
        }
    }
    /**
     * get kot tracking items by sale id
     * @access public
     * @param int $sale_id
     * @return array
     */
    public function getKotTrackingItemsBySaleId($sale_id) {
        ...
        foreach ($items as $item){
            if(!isset($item->menu_name) || $item->menu_name===''){
                $item->menu_name = getFoodMenuNameCodeById($item->food_menu_id);
            }
        }
        return $items;
    }
    ...
    public function getAllKitchenItemsAuto($sales_id,$printer_id,$kot_print=0){
        $this->db->select("tbl_kitchen_sales_details.menu_name,tbl_kitchen_sales_details.menu_note,tbl_kitchen_sales_details.qty,tbl_kitchen_sales_details.previous_id,tbl_kitchen_sales_details.food_menu_id,tbl_kitchen_sales_details.tmp_qty,tbl_kitchen_sales_details.menu_combo_items,tbl_kitchen_sales_details.id as sales_details_id,tbl_kitchen_sales_details.is_print,COALESCE(tbl_kitchen_sales_details.print_count,0) as print_count,tbl_printers.id as printer_id");
        $this->db->from('tbl_kitchen_sales_details');
        $this->db->join('tbl_food_menus', 'tbl_food_menus.id = tbl_kitchen_sales_details.food_menu_id', 'left');
        $this->db->join('tbl_kitchen_categories', 'tbl_kitchen_categories.cat_id = tbl_food_menus.category_id', 'left');
        $this->db->join('tbl_kitchens', 'tbl_kitchens.id = tbl_kitchen_categories.kitchen_id', 'left');
