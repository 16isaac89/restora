(function ($) {
  ...
  function get_total_vat() {
    ...
    $(".sidebar-cart-card").each(function () {
      ...
      let tax_information = [];
      if (menu_details && menu_details[0] && menu_details[0].tax_information) {
        try {
          tax_information = JSON.parse(menu_details[0].tax_information);
        } catch (e) {
          tax_information = [];
        }
      }
  ...
  function getFoodMenuTax(details_item_price,food_menu_id){
      ...
      let tax_information = [];
      if (menu_details && menu_details.length && menu_details[0].tax_information) {
        try {
          tax_information = JSON.parse(menu_details[0].tax_information);
        } catch (e) {
          tax_information = [];
  ...
  $(document).on("click", ".pay_now", function (e) {
    ...
    var outlet_id =  Number($("#online_selected_outlet_updated").val());
  ...
  $(".div_eye1").css("cursor","pointer");
  $(".div_eye2").css("cursor","pointer");

  $('.set_hide_icon').on('click', function() {
    const input = $('#password');
    const eye1 = $(this).find('.div_eye1');
    const eye2 = $(this).find('.div_eye2');

    if (input.attr('type') === 'password') {
      input.attr('type', 'text');
      eye1.addClass('display_none');
      eye2.removeClass('display_none');
    } else {
      input.attr('type', 'password');
      eye2.addClass('display_none');
      eye1.removeClass('display_none');
    }
  });
